<x-app-layout>
    <x-slot name="title">Edit Message Template</x-slot>

    <!-- PageHeading -->
    <div class="flex flex-wrap justify-between gap-3 items-center">
        <h1 class="text-gray-900 dark:text-white text-2xl sm:text-3xl lg:text-4xl font-black leading-tight tracking-[-0.033em]">Edit Message Template</h1>
    ...
    <div class="mt-6 sm:mt-8">
        <div class="w-full bg-white dark:bg-[#111a22] rounded-xl border border-gray-200 dark:border-[#324d67] p-4 sm:p-6">
            <form action="{{ route('admin.message-templates.update', $messageTemplate) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $messageTemplate->name) }}" required maxlength="255" class="w-full rounded-lg border border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent h-10 px-4 text-base font-normal leading-normal" placeholder="e.g., Welcome Message">
                </div>
                ...
                <div>
                    ...
                    <select name="category" id="category" required class="w-full rounded-lg border border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent h-10 px-4 text-base font-normal leading-normal">
                        ...
                        @foreach($allCategories as $cat)
                            <option value="{{ $cat }}" {{ old('category', $messageTemplate->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description', $messageTemplate->description) }}" maxlength="500" class="w-full rounded-lg border border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent h-10 px-4 text-base font-normal leading-normal" placeholder="Brief description of when to use this template">
                </div>

                <!-- Subject -->
                <div>
                    <label class="block text-gray-700 dark:text-gray-300 text-sm font-medium mb-2">Subject</label>
                    <input type="text" name="subject" id="subject" value="{{ old('subject', $messageTemplate->subject) }}" required maxlength="255" class="w-full rounded-lg border border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent h-10 px-4 text-base font-normal leading-normal" placeholder="Email subject line">
                ...
                <div>
                    ...
                    <input type="text" name="variables" id="variables" value="{{ old('variables', $messageTemplate->variables ? implode(', ', $messageTemplate->variables) : '') }}" class="w-full rounded-lg border border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent h-10 px-4 text-base font-normal leading-normal" placeholder="name, date, student_id (comma-separated)">
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $messageTemplate->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-[#324d67] bg-white dark:bg-[#111a22] text-primary focus:ring-primary">
